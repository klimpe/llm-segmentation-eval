...
_RULES: list[tuple[str, str, str]] = [
    ...
    # A verified, pre-existing corpus corruption (reports/phase2.md S4): a
    # capital-E-initial word turned into scientific notation by (almost
    # certainly) a spreadsheet auto-format artifact in the file's editing
    # history, in 3 files. Named, logged exception, same principle as the
    # reader's NUL-byte handling: strip, never reconstruct the lost letter.
    ("drop", "float_artifact_named_exception", r"0\.000000[eE]\+00(?=[^\W\d_])"),
    ...
    ("cue", "pitch_slash", r"/"),
    ("cue", "pitch_underscore", r"_"),
    ("boundary", "period", r"\."),
    ("boundary", "comma", r","),
    ("boundary", "qmark", r"\?"),
    # A written word: Unicode letters (so SBC037's Spanish text tokenises,
    # not just ASCII), internal apostrophe/hyphen for contractions and
    # compounds, an optional single trailing hyphen for word truncation
    # (y-) -- guarded so a real IU-truncation "--" right after a word
    # (no space) isn't half-eaten by this rule -- or trailing apostrophe
    # for a plural possessive (kids'). An optional LEADING apostrophe
    # handles a contraction split by a removed delimiter with no space,
    # e.g. "that]'s" (a numbered-overlap bracket closing mid-word): the
    # fragment "'s" only ever fuses onto an already-open word (tokenize()'s
    # fusion logic), never starts one on its own in real data.
    ("frag", "word", r"'?[^\W\d_]+(?:['-][^\W\d_]+)*(?:-(?!-)|')?"),
]
